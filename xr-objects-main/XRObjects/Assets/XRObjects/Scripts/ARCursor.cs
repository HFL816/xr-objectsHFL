...
public class ARCursor : MonoBehaviour
{
  ...
  public List<GameObject> spawnedContainersList = new List<GameObject>();
  ...
  public List<string> spawnedObjectNamesInScene = new List<string>();
  [SerializeField] private List<string> allowedObjectLabels = new List<string> { "bottle", "cup", "bowl", "cell phone", "laptop", "mouse", "vase", "potted plant", "apple", "orange", "backpack", "handbag","tv" };
    ...
    if (mediaPipeActive)
    {
      ...
      if (detectedObjectListToAddToScene?.Count > 0)
      {
        ...
        foreach (Detection detectedObject in detectedObjectListToAddToScene)
        {
          ...
          Debug.Log("Starting: " + detectedObject.Label[0]);
          ...
          else
          {
            ...
            Debug.Log(detectedObject.Label[0]+" too close, skipping");
            continue;
          }
          ...
          if (allowedObjectLabels.Contains(detectedObjectName))
          {
            ...
            RaycastHit[] hitGameObjects = Physics.RaycastAll(Camera.main.ScreenPointToRay(screenPosition));
            if (hitGameObjects.Length > 0)
            {
              
              foreach(RaycastHit rch in hitGameObjects){
                if(rch.collider.tag == "RealObjectProxy" || rch.collider.tag == "RealObjectSphere"){
                  found = true;
                }
              }
              if(found){
                Debug.Log(detectedObject.Label[0] + " raycast skip");
                continue;
              }
            }

            Collider[] sphereoverlap = Physics.OverlapSphere(Camera.main.ScreenToWorldPoint(screenPosition),0.2f);
            if(sphereoverlap.Length > 0){
              foreach(Collider c in sphereoverlap){
                if(c.tag == "RealObjectProxy" || c.tag == "RealObjectSphere"){
                  found = true;
                }
              }
              if(found){
                Debug.Log(detectedObject.Label[0] + " spherecast skip");
                continue;
              } 
            }

            Debug.Log(detectedObject.Label[0] + " made it through");
            Debug.Log("\n");
            ...
            if (hits.Count > 0)
            {
              ...
              newContainer.GetComponent<ImageQuery>().RunInitialImageQueryHFL();
  ...
  public void OutputCallback(object stream, OutputEventArgs<List<Detection>> eventArgs)
  {
    ...
    if (objectCount > 0)
    {
      ...
      {
        var detectedObjectName = detectedObject.Label[0];
        Debug.Log("Detected object label: " + detectedObjectName + "(" + detectedObject.Score[0] + ")");
      }
    }
  }
